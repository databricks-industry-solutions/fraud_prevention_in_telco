...
async def health():
    ...
    try:
        ...
        return JSONResponse(
            status_code=503,
            content={"status": "error", "detail": "Database connectivity check failed"},
        )



# ── App config (workspace host for dashboard embedding) ──────

@app.get("/api/config")
async def config():
    """Return workspace host and UI labels for the frontend.

    The frontend reads app_title at load time to set document.title,
    so the page title is configurable without a frontend rebuild.
    """
    return {
