  ...
  return (
    <div className="relative space-y-0">
      ...
      {history.map((item, idx) => {
        const config = decisionConfig[item.decision ?? ''] ?? { icon: Clock, color: 'text-gray-400', bg: 'bg-gray-500/20' }
        const Icon = config.icon
        const date = item.decided_at ? new Date(item.decided_at).toLocaleString() : 'Unknown date'

        return (
          <div key={item.decision_id || idx} className="relative pl-8 pb-4">
            {/* Timeline dot */}
            ...
            <div className="bg-gray-800/50 rounded px-3 py-2">
              <div className="flex items-center justify-between mb-1">
                <span className={`text-xs font-medium ${config.color}`}>
                  {(item.decision ?? 'unknown').replace(/_/g, ' ')}
                </span>
                <span className="text-[10px] text-gray-500">{date}</span>
              </div>
              <div className="text-[10px] text-gray-400 mb-1">
                by {item.analyst_name || 'Unknown'} · confidence: {item.confidence_level || 'N/A'}
              </div>
              {item.notes && (
