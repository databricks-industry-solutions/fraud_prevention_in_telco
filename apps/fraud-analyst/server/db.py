...
PGHOST = os.environ.get(
    "PGHOST",
    "instance-6f667f8e-b765-4f5e-b450-e6a87e47a731.database.cloud.databricks.com",
)
PGDATABASE = os.environ.get("PGDATABASE", "databricks_postgres")
PGSCHEMA = os.environ.get("PGSCHEMA", "fraud_data")
PGPORT = int(os.environ.get("PGPORT", "5432"))
PGUSER = os.environ.get("PGUSER", "")
LAKEBASE_INSTANCE = os.environ.get("LAKEBASE_INSTANCE", "telecom-fraud-lakebase")
...
class LakebasePool:
    ...
    async def _resolve_credentials(self) -> tuple[str, str]:
        ...
        if IS_DATABRICKS_APP:
            user = PGUSER
            # Use the SP's OAuth token directly as PG password
            try:
                ...
                try:
                    ...
                    logger.warning("Credential API also failed, trying secret scope")
                    try:
                        password = _fetch_secret("fraud-analyst", "pg-password")
                    except Exception:
...
class DeltaWriter:
    ...
    async def execute(
        ...
        """Execute a write query via SQL Statement API."""
        host = get_workspace_host()
        token = get_oauth_token()
